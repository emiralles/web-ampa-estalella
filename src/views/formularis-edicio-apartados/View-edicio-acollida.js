...
function ViewEdicioAcollida() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (inputAux.value !== "") {
            ...
            await updateOneDocOfTpo('acollida',idCard,dataQuisom);
    ...
    const handleEdit = ({target:{name}}) =>{
        ...
        let promise = getOneDocOfTipo('acollida',name);
        promise.then((result)=>{
    ...
    const handleRemove = ({target:{name}}) =>{
        ...
        let id = arrStr[0];
        deleteOneDocOfTipo('acollida',id);
        refresh();
    }
